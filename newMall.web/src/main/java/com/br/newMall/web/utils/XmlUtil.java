...
public class XmlUtil {
    ...
    public static void main(String[] args) throws DocumentException, IOException {
        String xmlStr = "<xml><ToUserName><![CDATA[gh_2797b2192c4f]]></ToUserName><FromUserName><![CDATA[or4Gy0yPbrBJeWAUMDPR7mzUxmvg]]></FromUserName><CreateTime>1531205630</CreateTime><MsgType><![CDATA[text]]></MsgType><Content><![CDATA[3]]></Content><MsgId>6576478104726958268</MsgId></xml>";
        Map<String, Object> customMessageMap = Maps.newConcurrentMap();
        if (StringTypeUtil.isJson(xmlStr)) {         //json格式
            customMessageMap.putAll(com.alibaba.fastjson.JSONObject.parseObject(xmlStr.toString(), Map.class));
        } else if (StringTypeUtil.isXML(xmlStr.toString())) {  //xml格式
            customMessageMap.putAll(XmlUtil.xml2mapWithAttr(xmlStr.toString(), false));
        } else {                                              //json格式
            customMessageMap.putAll(com.alibaba.fastjson.JSONObject.parseObject(xmlStr.toString(), Map.class));
        }
        System.out.println(customMessageMap);
    }
